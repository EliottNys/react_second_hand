export const Logo = () => (
  //   <svg fill="none" height="36" viewBox="0 0 32 32" width="36">
  //     <path
  //       clipRule="evenodd"
  //       d="M17.6482 10.1305L15.8785 7.02583L7.02979 22.5499H10.5278L17.6482 10.1305ZM19.8798 14.0457L18.11 17.1983L19.394 19.4511H16.8453L15.1056 22.5499H24.7272L19.8798 14.0457Z"
  //       fill="currentColor"
  //       fillRule="evenodd"
  //     />
  //   </svg>
  <svg
    className="p-2"
    xmlns="http://www.w3.org/2000/svg"
    height="48"
    viewBox="0 0 48 48"
    width="48"
  >
    <title>SecondLife</title>
    <g id="_11_-_20" data-name="11 - 20">
      <g id="SecondLife">
        <path
          fill="currentColor"
          d="M22.686,35.316l-.146-.122a2.1,2.1,0,0,0-2.952.258l.7-.839a2.1,2.1,0,0,0-.258-2.952l-.146-.122a2.1,2.1,0,0,0-2.952.258l0,0c.052-.062.105-.115.157-.178l.545-.66a2.081,2.081,0,0,0-.262-2.943l-.147-.126a2.081,2.081,0,0,0-2.943.262l-.713.838a2.1,2.1,0,0,0-.251-2.954l-.147-.126a2.094,2.094,0,0,0-2.954.262L7.657,29.226a1.423,1.423,0,0,0-.115.157,2.094,2.094,0,0,0,.367,2.8l.146.126a2.1,2.1,0,0,0,2.955-.262A2.1,2.1,0,0,0,11.271,35l.147.115a2.091,2.091,0,0,0,2.944-.251l.006-.007v0a2.094,2.094,0,0,0,.258,2.951l.146.123a2.1,2.1,0,0,0,2.952-.259,2.1,2.1,0,0,0,.258,2.952l.146.123a2.1,2.1,0,0,0,2.952-.258l1.865-2.223A2.1,2.1,0,0,0,22.686,35.316Z"
        ></path>
        <path
          fill="currentColor"
          d="M34.041,15.447a3.977,3.977,0,0,1-.392-1A7.7,7.7,0,0,0,31.074,14H27.618a7.752,7.752,0,0,0-4.127,1.188l-4.679,2.938a1.984,1.984,0,0,0,1.8,3.52l9.046-3.6,9.4,8.03c.2-.153.466-.376.756-.633Z"
        ></path>
        <rect
          fill="currentColor"
          x="38.506"
          y="9.975"
          width="6.493"
          height="16.671"
          rx="2"
          transform="translate(-3.562 23.329) rotate(-30)"
        ></rect>
        <rect
          fill="currentColor"
          x="-2.088"
          y="15.064"
          width="16.671"
          height="6.493"
          rx="2"
          transform="translate(-12.734 14.566) rotate(-60)"
        ></rect>
        <path
          fill="currentColor"
          d="M35.922,31.894l.516.48a2.223,2.223,0,0,0,2.9,0h0a2.222,2.222,0,0,0,0-3.368L30.02,20.982l-.742-.633L21.359,23.5a3.985,3.985,0,0,1-3.612-7.069l4.146-2.6-4.629-.577a8.77,8.77,0,0,0-2.782.11,3.961,3.961,0,0,1-.524,2.086L8.117,25.566l.574-.685a4.1,4.1,0,0,1,5.769-.511l.164.14a4.048,4.048,0,0,1,.8.912,4.08,4.08,0,0,1,3.1.935l.16.137a4.044,4.044,0,0,1,1.432,2.767c0,.036.006.071.008.106a4.1,4.1,0,0,1,1.051.64l.144.122a4.081,4.081,0,0,1,1.454,2.892,4.054,4.054,0,0,1,1.053.641l.149.125a4.065,4.065,0,0,1,.632,5.576l.452.412a2.182,2.182,0,0,0,3.1-.146,2.2,2.2,0,0,0-.147-3.091l.7.64a2.19,2.19,0,1,0,2.955-3.234l1.3,1.185a2.191,2.191,0,0,0,2.955-3.235Z"
        ></path>
      </g>
    </g>
  </svg>
);
